program g2ej3b
    ...
    INTEGER, PARAMETER :: cant_ecu=6, max_itera=75
    ...
    FUNCTION v_prima(v)
        ...
        v_prima(0) = 1.0
        v_prima(1) = v(2)
        v_prima(2) = (0.2-0.1*v(2)-1*v(1)+0.1*v(4)+1*v(3))/1
        v_prima(3) = v(4)
        v_prima(4) = (1*v(1)+0.1*v(2)+1*v(5)-(1+1)*v(3)-0.1*v(4))/1
        v_prima(5) = v(6)
        v_prima(6) = (0+1*v(3)-0.1*v(6)-v(5)*(1+1))/1
    ...
    SUBROUTINE RungeKuttaFehlberg(vi, cant_ec, max_iter, h)
        ...
        DO i=1, max_iter
            ...
            WRITE (*, '(I5, 7F16.10)') iter, v
            WRITE (2, '(I5, 7F16.10)') iter, v
        END DO
        close(2,status='keep')
