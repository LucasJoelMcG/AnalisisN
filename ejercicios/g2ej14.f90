program g2ej3b
    ...
    INTEGER, PARAMETER :: cant_ecu=3, max_itera=75
    ...
    FUNCTION v_prima(v)
        ...
        !v(0)=x v(1)=y
        REAL(8), DIMENSION(0:3) :: v, v_prima
        v_prima(0) = 1.0
        v_prima(1) = v(2)
        v_prima(2) = v(3)
        v_prima(3) = -2*v(3)+v(2)+2*v(1)+exp(v(0))
    ...
    SUBROUTINE RungeKuttaFehlberg(vi, cant_ec, max_iter, h)
        ...
        open(2,file='datos.txt',status='replace')
        WRITE (*, '(I5, 5F10.6)') 0, vi, 0.0
        WRITE (2, '(I5, 5F10.6)') 0, vi, 0.0
        v = vi
        DO i=1, max_iter
            k1 = h*v_prima(v)
            ...
            WRITE (*, '(I5, 5F16.10)') iter, v
            WRITE (2, '(I5, 5F16.10)') iter, v
        END DO
        close(2,status='keep')
