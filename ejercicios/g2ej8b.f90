program g2ej3b
    ...
    INTEGER, PARAMETER :: cant_ecu=1, max_itera=50
    ...
    call RungeKuttaFehlberg(vii,cant_ecu,max_itera,h1,tolerancia)
    ...
    FUNCTION v_prima(v)
        ...
        v_prima(0) = 1.0
        v_prima(1) = 3.0*v(1)+exp(1.0-v(0))
    END FUNCTION v_prima

    SUBROUTINE RungeKuttaFehlberg(vi, cant_ec, max_iter, h, tol)
        ...
        WRITE (*, '(I5, 2F10.6, 2F15.7)') 0, vi, 0.0, h
        WRITE (2, '(I5, 2F10.6, 2F15.7)') 0, vi, 0.0, h
        v = vi
        DO iter = 1, max_iter
            k1 = h*v_prima(v)
            ...
            h=h*(tol/(maxval(abs(e))))**alfa
            WRITE (*, '(I5, 2F22.6, 2F22.19)') iter, v, maxval(abs(e)), h
            WRITE (2, '(I5, 2F22.6, 2F22.19)') iter, v, maxval(abs(e)), h
        END DO
        CLOSE(2,status='keep')
